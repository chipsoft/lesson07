...
static int delay_in_ms = 100; // Задержка для таймера
static int restart = 20; // Число вызовов HR таймера
static struct hrtimer hr_timer; // Глобальная структура HR таймера

// Функция HR таймера, которая вызывается по CALLBACK
...
static int jit_hrtimer_proc_show(void)
{
	printk(KERN_INFO "jit_hrtimer_proc_show\n");
	ktime_t ktime;
